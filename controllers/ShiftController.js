...
exports.createShift = async (req, res) => {
  try {
    ...
    if (!shiftName || !daysOfWeek) {
      return res
        .status(400)
        .json({ message: "Shift name and days of week are required" });
    }

    // Check if shift with same name already exists
    const existingShift = await Shift.findOne({ shiftName: shiftName.trim() });
    if (existingShift) {
      return res
        .status(409)
        .json({ message: `Shift with name "${shiftName}" already exists` });
    }

    // Validate each day
    for (const day of Object.keys(daysOfWeek)) {
      const info = daysOfWeek[day];
      if (info.dayOff) {
        info.attendanceStart = null;
        info.attendanceEnd = null;
        info.breakStart = null;
        info.breakEnd = null;
        info.noBreak = null;
      } else {
        const {
        ...
        if (!attendanceStart || !attendanceEnd) {
          return res.status(400).json({
            message: `Attendance start and end times are required for ${day}`,
          });
        }
        if (!noBreak && (!breakStart || !breakEnd)) {
          return res.status(400).json({
            message: `Break start and end times are required for ${day}`,
          });
        }
      }
    }
    ...
    // Create and save
    const newShift = new Shift({ shiftName: shiftName.trim(), daysOfWeek });
    await newShift.save();
